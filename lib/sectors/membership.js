...
// atribuidos. Assignees ATUAIS sao grandfathered — um chamado movido de setor
// mantem o responsavel, e editar a lista depois nao pode travar por causa dele.
// Devolve os ids invalidos (vazio = tudo ok). Puro; o espelho do front filtra
// as OPCOES do select, esta regra e quem decide de verdade no servico.
function findInvalidAssignees({ requestedIds, memberIds, currentIds }) {
	const allowed = new Set([...(memberIds || []), ...(currentIds || [])]);
	return (requestedIds || []).filter((id) => !allowed.has(id));
}

module.exports = { validateMemberChange, findInvalidAssignees };
