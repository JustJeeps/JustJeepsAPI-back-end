...
// 120s de proposito, e SEM herdar o MAGENTO_TIMEOUT_MS global (15-20s): em
// producao o POST bulk leva ~35s sob carga (medido em 2026-08-25, 8 lotes em
// 280s), e um timeout curto transforma cada lote num desfecho ambiguo — que
// este pipeline trata como veneno. Sync de reviews e um job de fundo: esperar
// e barato, ambiguidade e cara.
const REVIEWS_TIMEOUT_DEFAULT_MS = 120000;
...
function toMagentoError(error) {
	...
	// 404 no GET de reviews = o PRODUTO nao existe na loja — resposta
	// definitiva (review de produto inexistente nao pode ter sido gravada),
	// diferente dos demais 4xx. A recuperacao usa isso para nao bloquear.
	if (status === 404) return make('MAGENTO_NOT_FOUND', true, `Magento says the product does not exist${detail ? `: ${detail}` : ''}`);
...
function createMagentoReviewsClient({ http = axios, env = process.env } = {}) {
	...
	const timeout = Number(env.MAGENTO_REVIEWS_TIMEOUT_MS || REVIEWS_TIMEOUT_DEFAULT_MS);
