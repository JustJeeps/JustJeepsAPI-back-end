...
// update; we just refuse to remove rows on the strength of a thin run. The
// floor protects the rows already in the table: a run that matches far fewer
// than what is stored must not delete.
function checkStaleFloor({ matched = 0, existingCount = null, thresholds } = {}) {
	const t = withDefaults(thresholds);
	if (matched < t.minMatched) return { ok: false, reason: `matched ${matched} is below minMatched ${t.minMatched}` };
	if (existingCount !== null && existingCount !== undefined && matched < existingCount * t.matchDropRatio) {
		return { ok: false, reason: `matched ${matched} dropped below ${t.matchDropRatio} of the rows already stored (${existingCount})` };
	}
