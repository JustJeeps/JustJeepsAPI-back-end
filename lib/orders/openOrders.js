...
//
// An order is OPEN by OUR ship status only (custom_ship_status, the field the
// team keeps in the Pricing Tool): done list below = closed, anything else,
// empty included, = open. Magento's status is not part of the rule (decided
// 2026-09-23; before that it decided when our field was empty). Magento never
// closes drop-shipped orders (1,077 "processing" orders were already Drop
// Shipped), and the PO is not part of the rule either (with "PO not set" only
// 21 orders in the store were open and the tag never showed).
//
// Magento's status is used only for the warning: when it and our ship status
// disagree, getStatusDivergence says so and the Orders screen shows a warning
...
const isOpenOrder = (order) => !isShipStatusDone(order?.custom_ship_status);
...
// `custom_ship_status: null` is listed explicitly: SQL `NOT IN` drops null rows.
