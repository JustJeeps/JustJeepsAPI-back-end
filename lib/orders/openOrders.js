...
//
// An order is OPEN by OUR ship status (custom_ship_status, the field the team
// keeps): when it has a value, it decides (done list below = closed, anything
// else = open). Only when it is empty does Magento's status decide (complete,
// closed, canceled = closed; null or unknown = open). Decided 2026-09-16:
// Magento never closes drop-shipped orders (1,077 "processing" orders were
// already Drop Shipped), and the PO is not part of the rule either (with
// "PO not set" only 21 orders in the store were open and the tag never showed).
//
// When Magento and our ship status disagree, getStatusDivergence says so and
// the Orders screen shows a warning next to the order number.
//
...
// The rule. The SQL below only pre-filters; this decides.
const isOpenOrder = (order) => {
	if (hasShipStatus(order?.custom_ship_status)) return !isShipStatusDone(order.custom_ship_status);
	return !isMagentoStatusClosed(order?.status);
};
...
// `status: null` and `custom_ship_status: null` are listed explicitly:
// SQL `NOT IN` drops null rows.
const buildOpenOrdersWhere = () => ({
	OR: [
		{
			AND: [
				{ custom_ship_status: { not: null } },
				{ custom_ship_status: { notIn: [...EMPTY_SHIP_STATUSES] } },
				{ NOT: { custom_ship_status: { in: [...DONE_SHIP_STATUSES], mode: 'insensitive' } } },
			],
		},
		{
			AND: [
				{ OR: [{ custom_ship_status: null }, { custom_ship_status: { in: [...EMPTY_SHIP_STATUSES] } }] },
