...
async function collectRequestsDigestData(prisma, { since = null, now = new Date() } = {}) {
	...
	const [newRequests, updates, unassigned, aging] = await Promise.all([
		prisma.request.findMany({
			where: { createdAt: { gt: windowStart }, deletedAt: null },
			include: REQUEST_INCLUDE,
			orderBy: { createdAt: 'desc' },
		}),
		prisma.requestActivity.findMany({
			// Entradas "created" ja aparecem na secao de novas solicitacoes.
			// O request deletado sai junto: sem isto, a propria activity
			// 'deleted' (e titulo/nome de anexo) vazaria no e-mail do digest.
			where: {
		...
		prisma.request.findMany({
			where: { assignee_id: null, status: { not: 'Closed' }, archivedAt: null, deletedAt: null },
			include: REQUEST_INCLUDE,
			orderBy: { createdAt: 'asc' },
		}),
		prisma.request.findMany({
			where: { status: { notIn: ['Closed'] }, updatedAt: { lt: agingCutoff }, archivedAt: null, deletedAt: null },
			include: REQUEST_INCLUDE,
...
async function collectRequestsWeeklyStatusData(prisma, { now = new Date() } = {}) {
	...
	const archivedCount = requests.filter((request) => Boolean(request.archivedAt)).length;
	const closedCount = requests.filter((request) => String(request.status || '') === 'Closed').length;
	const openCount = requests.length - closedCount;
