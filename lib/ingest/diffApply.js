  ...
  if (staleStrategy !== "none") {
    ...
    if (staleStrategy === "delete") {
      await prisma.$executeRawUnsafe(`
        ...
        WHERE op.vendor_product_id IN (SELECT t.id FROM ${t} t WHERE ${staleWhere})
      `);
      const deletedRows = await prisma.$queryRawUnsafe(`
        WITH del AS (
          DELETE FROM ${t} t2 USING (SELECT t.id FROM ${t} t WHERE ${staleWhere}) victims
          WHERE t2.id = victims.id
          RETURNING 1
        ) SELECT count(*)::int AS n FROM del
      `);
      stale = deletedRows[0].n;
