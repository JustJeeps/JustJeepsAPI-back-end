...
async function listFeedStatuses(prisma, feedDefinitions, { now = new Date() } = {}) {
	...
	for (const feed of feedDefinitions) {
		...
		// The consuming seed may record its run under a different name than the
		// registry uses (see ingestFeed in config/feeds.js).
		const ingestFeed = feed.ingestFeed || feed.name;
		const [lastRun, lastFetch] = await Promise.all([
			prisma.ingestRun.findFirst({ where: { feed: ingestFeed }, orderBy: { id: 'desc' } }),
