...
async function registerArtifacts(prisma, { feed, batchId, source, uploadedBy = null, note = null, files }) {
	...
	// Two entries with the same name in one batch would both be "available" for
	// that name, and the materializer writes each one to the same canonical path,
	// so which content the vendor script reads would come down to row order.
	// Caught in production by a shell glob that expanded to two cached copies of
	// the same file.
	const duplicated = fileNames.filter((name, index) => fileNames.indexOf(name) !== index);
	if (duplicated.length > 0) {
		throw new Error(
			`registerArtifacts: the same file appears more than once in the batch (${[...new Set(duplicated)].join(', ')})`
		);
	}
		...
		for (const file of files) {
			rows.push(await tx.feedArtifact.create({
				data: {
					...
					// When the file was produced at the source, when we know it.
					// Read from the uploader's disk, not from ours: our copy's
					// mtime is the moment we downloaded it.
					sourceModifiedAt: file.sourceModifiedAt ? new Date(file.sourceModifiedAt) : null,
					source,
					uploadedBy,
					note,
				},
...
function pickCurrentBatch(artifacts, expectedFileNames) {
	...
	for (const [batchId, batchArtifacts] of byBatch) {
		...
		if (expectedFileNames.every((name) => names.has(name))) {
			...
			// How old the DATA is, which is not how long ago we received it. A
			// spreadsheet exported in March and uploaded today is five months
			// old, and reading the upload time called it fresh. Only as fresh as
			// the oldest file in the batch, since a partial upload mixes a new
			// file with one carried forward.
			const dataAsOf = batchArtifacts
				.map((a) => a.sourceModifiedAt || a.uploadedAt)
				.sort((a, b) => a - b)[0];
			return { batchId, uploadedAt, dataAsOf, artifacts: batchArtifacts };
		}
	}
	return null;
}
...
// to the next upload as "already stored, nothing to send". Purged rows are
// excluded too: retention deleted their object from the bucket, so offering
// their key as "already stored" would point the new artifact at a dead key.
async function findArtifactByHash(prisma, feed, fileName, sha256) {
	return prisma.feedArtifact.findFirst({
		where: { feed, fileName, sha256, status: { notIn: ['quarantined', 'purged'] } },
		orderBy: { uploadedAt: 'desc' },
	});
}
...
// Purged rows stay purged: their object is gone, so relabeling them as
// quarantined would misrepresent what is actually in the bucket.
async function quarantineBatch(prisma, batchId, note) {
	return prisma.feedArtifact.updateMany({
		where: { batchId, status: { not: 'purged' } },
		data: { status: 'quarantined', ...(note ? { note } : {}) },
	});
}

// excludeFeeds keeps the runs of a restricted feed (the QuickBooks export) out
// of the unfiltered history for anyone who cannot see the feed itself.
async function listRuns(prisma, { feed, status, limit = 50, offset = 0, excludeFeeds = [] } = {}) {
	const where = {
		...(feed ? { feed } : {}),
		...(status ? { status } : {}),
		...(excludeFeeds.length > 0 && !feed ? { feed: { notIn: excludeFeeds } } : {}),
	};
...
async function listFeedStatuses(prisma, feedDefinitions, { now = new Date() } = {}) {
	...
	return feedDefinitions.map((feed) => {
		...
		// Age of the data, not of the delivery (see dataAsOf in pickCurrentBatch).
		const ageHours = currentBatch ? (now - currentBatch.dataAsOf) / 36e5 : null;
