...
function createLegacySync({
	...
	async function syncFeed(feed) {
		...
		return {
			...
			// Where the links actually landed, which is not always under api-calls.
			linkDir: path.dirname(links[0]?.linkPath || path.resolve(apiCallsDir, feed.legacyDir || '')),
			links,
		};
	}
	...
	async function syncAllFeeds() {
		...
		for (const feed of feedsConfig.getFeedDefinitions()) {
			try {
				...
				synced.push(result);
				logger.log(
					`🔗 [feed-sync] ${feed.name}: batch ${result.batchId}${result.stale ? ' (STALE)' : ''} -> ${feed.files.length} file(s) in ${result.linkDir}`
				);
			} catch (error) {
