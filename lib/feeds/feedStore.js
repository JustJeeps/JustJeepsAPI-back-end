...
// History is bounded, not eternal: the retention prune (lib/feeds/retention.js
// + scripts/feed-prune.js) keeps the newest versions per file and deletes the
// rest, guided by the FeedArtifact catalog.
//
...
function createFeedStore({ s3 = null, env = process.env } = {}) {
	...
	// Pagina até o fim: uma ListObjectsV2 devolve no máximo 1000 chaves, e tanto
	// o prune de feeds quanto o run-log archive já podem passar disso.
	async function listObjects(prefix) {
		const items = [];
		let continuationToken;
		do {
			const output = await getClient().send(new ListObjectsV2Command({
				Bucket: bucket,
				Prefix: prefix,
				ContinuationToken: continuationToken,
			}));
			for (const item of output.Contents || []) {
				items.push({ key: item.Key, size: item.Size, lastModified: item.LastModified });
			}
