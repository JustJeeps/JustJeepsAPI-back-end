...
async function downloadVerified(store, artifact, batchDir) {
	...
	const attempt = async () => {
		...
		console.log(`⬇️ [feeds] downloading ${artifact.fileName} (${formatMb(totalBytes)})...`);
		...
		console.log(`✅ [feeds] ${artifact.fileName} downloaded (${formatMb(receivedBytes)}), verifying hash...`);
	...
	for (let attemptNumber = 1; attemptNumber <= 2; attemptNumber += 1) {
		...
		console.warn(`⚠️ [feeds] hash mismatch on ${artifact.objectKey} (attempt ${attemptNumber})`);
	}
	throw typedError(
		'FEED_HASH_MISMATCH',
