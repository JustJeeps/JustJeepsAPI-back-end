...
function createKeystoneFtpClient({
	...
	const verifyRemoteSize = async (client, remoteFile, localPath) => {
		...
		try {
			...
			console.warn(`⚠️ Could not read the size of ${remoteFile} at the vendor, skipping the size check: ${error.message}`);
			return;
		}
	...
	async function downloadFile(remoteFile, localPath) {
		...
		while (stalledAttempts < maxAttempts) {
			...
			try {
				...
				console.log(`📥 Downloading ${remoteFile} (attempt ${attempt})${modifiedAt ? `, published ${modifiedAt.toISOString()}` : ''}...`);
