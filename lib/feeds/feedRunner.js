...
// seed-all takes about 25 minutes; a lock older than this is a leftover.
const SEED_ALL_LOCK_MAX_AGE_MS = Number(process.env.SEED_ALL_LOCK_MAX_AGE_MS || 3 * 60 * 60 * 1000);
...
function createFeedRunner({
	...
	async function recordRunIfSeedDidNot(feed, record) {
		if (!prisma) return;
		// A fetch already records itself under "<feed>-fetch" (keystoneFetchService),
		// and writing a row here would file the result of going to the vendor as if
		// it were an ingest of the feed.
		if (record.mode === 'fetch') return;
	...
	// mode 'seed'  -> feed-sync + the scripts that consume the feed
	// mode 'fetch' -> go get the file at the vendor now (Keystone FTP)
	//
	// The fetch exists as a button because the schedule is a guess: it runs at
	// 4:47 and 16:47 and the vendor sometimes has not published the file yet, so
	// the run succeeds having downloaded yesterday's data and nobody can do
	// anything about it until the next window.
	function start(feedName, { startedBy = null, mode = 'seed' } = {}) {
		const feed = feedsConfig.getFeedByName(feedName);
		if (!feed) throw typedError('FEED_UNKNOWN', `Unknown feed: ${feedName}`);
		if (mode === 'fetch' && !feed.fetchCommand) {
			throw typedError('FEED_RUN_NOT_ALLOWED', `Feed ${feedName} is not fetched from a vendor`);
		}
		if (mode !== 'fetch' && !feed.seedCommand) {
		...
		// The lock file is removed by seed-all when it ends, which does not happen
		// if the container is replaced mid-run. Without an age check the button
		// answered "the daily sync is running" forever, with no way to clear it.
		if (fs.existsSync(seedAllLockFile)) {
			const lockAgeMs = now() - fs.statSync(seedAllLockFile).mtimeMs;
			if (lockAgeMs < SEED_ALL_LOCK_MAX_AGE_MS) {
				throw typedError('FEED_RUN_BUSY', 'The daily vendor sync is running. Try again when it finishes.');
			}
			console.warn(`Ignoring a seed-all lock left behind ${Math.round(lockAgeMs / 60000)} min ago`);
		}

		// A feed may need more than one script, in the order the daily sync runs
		// them (Quadratec applies prices first, then inventory).
		const commands = mode === 'fetch'
			? [feed.fetchCommand]
			: (Array.isArray(feed.seedCommand) ? feed.seedCommand : [feed.seedCommand]);
		...
		// The fetch PRODUCES the batch, so syncing before it would be pointless
		// (and would fail on a feed that has none yet).
		const shellCommand = mode === 'fetch'
			? `npm run ${commands[0]}`
			: [`npm run feed-sync -- ${feed.name}`, ...commands.map((command) => `npm run ${command}`)].join(' && ');
		...
		const finish = (exitCode, error) => {
			...
			// A bare non-zero exit used to be recorded with no message at all, so
			// the history showed "failed" with an empty Error column.
			if (record.status === 'failed' && !record.error) {
				record.error = exitCode === null
					? 'The process ended without an exit code'
					: `Exit code ${exitCode} (see the run log for details)`;
			}
			activeFeed = null;
	...
	// Off the event loop on purpose: the panel polls this every 2 seconds per
	// open tab while a script runs, and a synchronous read blocks every other
	// request on a box with a single vCPU.
	...
	// Everything except the log tail, which the caller may not need.
	function getRun(feedName) {
	...
	async function getStatus(feedName) {
		const run = getRun(feedName);
		if (!run) return null;
		return { ...run, logTail: await readLogTail(run.logFile) };
	}

	const isBusy = () => Boolean(activeFeed);
	const busyFeed = () => activeFeed;

	// The panel asks so it can disable the button with a reason instead of
	// letting the user press it and receive a raw 409.
	const isDailySyncRunning = () => {
		try {
			if (!fs.existsSync(seedAllLockFile)) return false;
			return now() - fs.statSync(seedAllLockFile).mtimeMs < SEED_ALL_LOCK_MAX_AGE_MS;
		} catch {
			return false;
		}
	};

	return { start, getRun, getStatus, isBusy, busyFeed, isDailySyncRunning };
}
