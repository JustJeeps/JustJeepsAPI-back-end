...
- Only assignee managers can assign or unassign a request (`REQUESTS_ASSIGNEE_MANAGERS`; default `tess,tsantos`, now including `rafael` in deploy config). Only triage can close. Triage users come from `REQUESTS_TRIAGE_USERS` (default `ricardo,admin,tess`), exposed to the frontend as `meta.triageUsers`.
...
- Comment notification email: sent on each new comment to the requester, all followers and all assignees (deduped). Uses two templates: `Ticket closed + comment` when the same update transitions the ticket to Closed, and `New comment` for all other comment events. Kill switch `REQUESTS_COMMENT_EMAIL_ENABLED=false`.
