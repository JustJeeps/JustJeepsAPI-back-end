...
- **Only sector members can be assigned** (2026-08-14). The assignee selects offer only the request's sector members (`meta.sectors[].memberIds` — ids only; roles and Trello mapping stay admin-only) and the backend enforces it with 409 `ASSIGNEE_NOT_IN_SECTOR`. Current assignees are grandfathered so moving a request never locks its list. When one PATCH moves and assigns, the DESTINATION sector's membership is what counts.
