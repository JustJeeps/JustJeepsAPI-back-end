...
async function run() {
  console.log('Sales rep backfill starting...');
  ...
  while (!shouldStop) {
    ...
    for (const row of rows) {
      ...
      const currentValue = row.sales_rep ? String(row.sales_rep) : '';
      ...
      try {
        const label = await fetchSalesRepLabel(row.entity_id);
  ...
  console.log('Sales rep backfill complete.');
  console.log(
    JSON.stringify(
      {
        scanned,
