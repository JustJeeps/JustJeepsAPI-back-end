...
Requests feature models: `Request`, `RequestComment`, `RequestAttachment`, `RequestActivity`, `TrelloSettings`, `Sector`, `SectorMember`, `TrelloSectorBoard`, `SectorActivity` (see `docs/REQUESTS.md` and `docs/SECTORS.md`; `TrelloUserBoard` is retired, dormant until the cleanup migration). Product replacement models: `ProductReplacement`, `ProductReplacementComment` (see `docs/PRODUCT-REPLACEMENTS.md`). Their migrations add two partial unique indexes (`ProductReplacement_active_pair_key` for active pairs, `ProductReplacement_active_none_key` for the active "no replacement" marker) that Prisma cannot express in the schema: a future `migrate diff` will propose dropping them; that proposal is wrong. Other models: `QuickBooksImport`, `QuickBooksCustomer`, `SyncState`, `IngestRun`, `SkuStatusChangeHistory`, `OrderCancellationWorkflowHistory`.
