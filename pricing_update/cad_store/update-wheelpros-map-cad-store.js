...
async function getWheelProsMapRows({ limit, sku }) {
  ...
  const products = await prisma.product.findMany({
    ...
    select: {
      ...
      MAP: true,
      vendorProducts: {
        where: { vendor_id: WHEELPROS_VENDOR_ID },
  ...
  for (const product of products) {
    ...
    rows.push({
      ...
      wheelpros_vendor_sku: product.vendorProducts?.[0]?.vendor_sku || null,
      ...
      map_source: 'product.MAP (seed-wheelPros)',
    });
  }
...
function printUsage() {
  console.log('Check and fix WheelPros CAD prices against Product.MAP.');
  console.log('MAP source behavior:');
  console.log('  - Uses active products linked to WheelPros vendor rows (vendor_id=5) and Product.MAP > 0');
  ...
  console.log('');
  console.log('Usage:');
  console.log('  node pricing_update/cad_store/update-wheelpros-map-cad-store.js [options]');
  console.log('');
  console.log('Options:');
...
async function main() {
  ...
  console.log('🚀 CAD Store WheelPros MAP Alignment');
  ...
  const { rows, scanned } = await getWheelProsMapRows({
    limit: options.limit,
    sku: options.sku,
  });
  ...
  console.log(`📦 WheelPros products scanned: ${scanned}`);
  console.log(`✅ Price rows computed: ${comparedRows.length}`);
  console.log(`🔍 Comparison source: ${comparisonSource}`);
  console.log(`📉 Below MAP: ${belowMapCount}`);
  console.log(`🟰 At MAP: ${atMapCount}`);
  console.log(`📈 Above MAP: ${aboveMapCount}`);
  ...
  if (unresolvedMagentoPriceCount > 0) {
    console.log(`⚠️ Could not read Magento price for ${unresolvedMagentoPriceCount} SKU(s); unresolved SKUs fallback to DB price`);
