...
async function buildQuadratecRows() {
  ...
  if (!Array.isArray(raw) || raw.length === 0) {
    return { rowsToInsert: [], rawCount: 0, missingProduct: 0 };
  }

  // 1) Clean + normalize + dedupe by quadratec_code (last occurrence wins)
  ...
  return { rowsToInsert, rawCount: raw.length, missingProduct };
}

const STAGING_TABLE = "vp_quadratec";
const STAGING_DDL = `
  ...
  quadratec_shipping_surcharge_usd DOUBLE PRECISION,
...
async function seedQuadratecStaged() {
  ...
  try {
    ...
    run = await startRun(FEED, {
      sourceKind: "csv",
      sourceRef: sourcePath.split("/").pop(),
      sourceHash,
    });
    ...
    for (let i = 0; i < rowsToInsert.length; i += BATCH_SIZE) {
      await insertBatch(STAGING_TABLE, STAGING_COLS, rowsToInsert.slice(i, i + BATCH_SIZE));
    }
    ...
    const counts = await diffApply({
      ...
      compareCols: [
        "product_sku", "vendor_cost", "vendor_cost_usd",
        "quadratec_shipping_surcharge_usd", "vendor_retail_price_usd", "quadratec_sku",
      ],
      insertCols: STAGING_COLS,
    ...
    console.log(`✅ VendorProduct count for Quadratec (vendor_id=${VENDOR_ID}): ${count}`);
  } catch (error) {
...
async function seedQuadratecLegacy() {
  ...
  try {
    const { rowsToInsert } = await buildQuadratecRows();
    ...
    for (let i = 0; i < rowsToInsert.length; i += BATCH_SIZE) {
      await prisma.vendorProduct.createMany({
        data: rowsToInsert.slice(i, i + BATCH_SIZE),
        skipDuplicates: true,
      });
    }
...
const seedQuadratec = process.env.QUADRATEC_PIPELINE === "legacy"
  ? seedQuadratecLegacy
  : seedQuadratecStaged;
